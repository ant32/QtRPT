...
MainWindow::MainWindow(QWidget *parent) :  QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    newContList = new QList<TContainerField *>();
    cloneContList = new QList<TContainerField *>();
...
void MainWindow::saveReport() {
    ...
    docElem.setAttribute("email","aliks-os@yandex.ru");
...
bool MainWindow::setXMLProperty(QDomElement *repElem, QWidget *widget) {
    ...
    if (band != 0) {
        ...
        elem.setAttribute("height",band->contWidget->geometry().height());
...
void MainWindow::clipBoard() {
    ...
    if (sender() == ui->actPaste) {
        if (cloneContList->size() == 0) return;
        ...
        for (int i=0; i<cloneContList->size(); i++) {
            ...
            newContField = cloneContList->at(i)->clone();
            generateName(newContField);
            ...
            newContField->setMenu(contMenu);
            newContField->setFocus();
            newContField->setSelected(false);
            newContField->setVisible(true);
